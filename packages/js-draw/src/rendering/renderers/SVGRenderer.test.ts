...
describe('SVGRenderer', () => {
	it('text highp and CSS transforms should match', () => {
		const testTransforms = [
			Mat33.identity,
			new Mat33(1, 2, 3.4567891, 3, 4, 5.67891011, 0, 0, 1),
			new Mat33(-1, -2, 3, 4, 5, 6, 0, 0, 1),
			new Mat33(1, 2, 0.00001, 3, 4, -0.00000123, 0, 0, 1),
		];
		...
		for (const transform of testTransforms) {
			...
			const transformFromProperty = Mat33.fromCSSMatrix(
				textElement.getAttribute('data-highp-transform') ?? '',
			);
