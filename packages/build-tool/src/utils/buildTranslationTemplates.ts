...
const localeToLanguageName = (locale: string) => {
	switch (locale) {
		case 'en':
			return 'English';
		case 'es':
...
const generateTranslationTemplate = (
	...
	if (destLocale === undefined) {
		...
		description = `Update the "${localeToLanguageName(destLocale)}" translation in ${projName}!`;
	}
...
title: "[Translation]: <language>${destLocale ? ` (${localeToLanguageName(destLocale)})` : ''}"
labels: [localization]
assignees: []
body:
...
const buildTranslationTemplate = async (source: TranslationSource, destFolder: string) => {
	...
	for (const locale in locales) {
		...
		console.log(
			'Building translation template for locale ',
			locale,
			'-',
