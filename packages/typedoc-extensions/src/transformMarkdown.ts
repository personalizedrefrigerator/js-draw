...
import constants from './constants.autogenerated';
...
const replaceMacros = (markdown: string) => {
	return markdown.replace(/{{!VERSION!}}/g, constants.version);
};

const transformMarkdown = (markdown: string, callbacks: Callbacks): string => {
	markdown = replaceMacros(markdown);
